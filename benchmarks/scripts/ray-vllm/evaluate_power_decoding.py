...
current_dir = os.path.dirname(os.path.abspath(__file__)) # benchmarks/scripts/ray-vllm
scripts_dir = os.path.dirname(current_dir) # benchmarks/scripts
benchmark_base_dir = os.path.dirname(scripts_dir) # benchmarks/
sys.path.append(benchmark_base_dir)
...
def main():
    ...
    model_config, infra_config, inference_config, generation_config, prompt_config, benchmark_config, pd_config = \
    ...
    console.print(f"Initializing RayVllmGenerator (Power Decoding Mode: {pd_config.use_power_decoding})", style=subhead_style_2)
    ...
    # 3. Generate text
    # Prepare PD config dict if enabled
    pd_config_dict = None
    if pd_config.use_power_decoding:
        pd_config_dict = {
    ...
    benchmark.run(
        ...
        # Pass PD config
        pd_config=pd_config_dict
    )

    # 4. Release GPU memory
    console.print("\nReleasing resources...", style=warning_style)
