#!/bin/bash
# bash eval_script_rag4_run_5090_temp.sh ../checkpoints/OneRec-1.7B results_1.7B true ../raw_data/onerec_data/benchmark_data_1000 "0 1 2 3 4 5 6 7"
...
MODEL_PATH=$1
VERSION="${VERSION:-a}"
# VERSION="${VERSION:-v1.0_1000_5090_ab_chazhi}"
...
ALPHA_ARGS=""
if [ -n "$CD_ALPHA" ]; then
    ALPHA_ARGS="--cd_alpha $CD_ALPHA"
fi
echo "Alpha args: $ALPHA_ARGS"
...
$PYTHON_EXEC -u scripts/ray-vllm/evaluate.py \
    ...
    --sample_size 10 \
    $THINKING_ARGS $ALPHA_ARGS >> "${BASE_LOG_NAME}.log" 2>&1
