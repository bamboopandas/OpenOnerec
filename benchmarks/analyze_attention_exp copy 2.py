    ...
    def plot_text_heatmap(tokens, attn, title, filename, v_max):
        ...
        with PdfPages(filename) as pdf:
            for p_idx, items in enumerate(pages):
                ...
                # But we want margins.
                # It's easier to turn off axis and position rects manually?
                # Or set ylim to match our abstract units.
                # USABLE_HEIGHT in inches maps to (lines_per_page * LINE_HEIGHT) units
                y_span_units = lines_per_page * LINE_HEIGHT
                
                # We want 0 at top-ish.
                # Let's set ylim so that y=0 is near top margin.
                ax.set_ylim(-y_span_units, LINE_HEIGHT) 
