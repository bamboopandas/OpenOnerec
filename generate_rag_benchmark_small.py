...
OUTPUT_BASE_DIR = "raw_data/onerec_data/benchmark_data_id_v4_cleandata_small"
...
SAMPLE_LIMIT = 50  # Limit output rows per file
INDEX_LIMIT = 50000 # Limit rows for indexing to save memory/time
...
class SidConverter:
    ...
    def to_sid_str(self, pids, domain='video', max_len=100, deduplicate=True):
        ...
        if len(pid_list) > max_len:
            # print(f"DEBUG: Truncating {len(pid_list)} to {max_len}")
    ...
    def save_mappings_json(self, output_dir):
        """Saves SID->PID mappings to JSON for evaluation."""
        print("Saving SID->PID mappings for evaluation...")
        
        # Constants for encoding (from benchmark/tasks/v1_0/recommendation/utils_by_pid.py)
        CODE_MULTIPLIER_1 = 8192 * 8192
        CODE_MULTIPLIER_2 = 8192
        
        def encode_sid(c0, c1, c2):
            return c0 * CODE_MULTIPLIER_1 + c1 * CODE_MULTIPLIER_2 + c2

        def save_domain_mapping(domain, filename, key_name="pid"):
            if domain not in self.maps: return
            
            pid2sid = self.maps[domain]
            sid2pids = defaultdict(list)
            
            # Invert: encoded_sid -> list of pids
            for pid, code in pid2sid.items():
                try:
                    # code is [c0, c1, c2]
                    encoded = encode_sid(code[0], code[1], code[2])
                    encoded_int = int(encoded)
                    # Construct info dict expected by evaluator
                    # We don't have real counts, defaulting to 1
                    info = {key_name: int(pid), "count": 1, "count_after_downsample": 1}
                    sid2pids[encoded_int].append(info)
                except Exception as e:
                    continue
            
            out_path = os.path.join(output_dir, filename)
            with open(out_path, 'w') as f:
                json.dump(sid2pids, f)
            print(f"  Saved {out_path} ({len(sid2pids)} encoded entries)")

        # Video/Ad Mapping (sid2pid.json)
        save_domain_mapping('video', "sid2pid.json", "pid")

        # Product Mapping (sid2iid.json)
        # Note: Product task usually uses 'iid' in the evaluator utils but 'pid' in data files.
        # utils_by_pid.py: _get_id_from_info uses info.get("pid", info.get("iid", 0))
        # So "pid" key is fine for both.
        save_domain_mapping('product', "sid2iid.json", "pid")
...
def fill_template(query_text, example_data, converter, max_len):
    ...
    # Regex to find all SID blocks (sequences of SIDs)
    ...
    for i in range(0, len(parts) - 1, 2):
        ...
        ex_sids_str = converter.to_sid_str(source_data, domain, max_len=current_limit, deduplicate=True)
...
def format_rag_entry(example, query_text, converter, task_type, max_len):
    ...
    for i in range(0, len(parts) - 1, 2):
        ...
        # Decide Data Source and Limit
        source = []
        domain = 'video'
        current_limit = 50 # Default fallback
        
        if "长时间" in prefix:
            source = data.get('hist_longview_video_list', [])
            current_limit = 100 # From data/onerec_data/sft/ad_rec.py and product_rec.py
        elif "商品" in prefix or "购物" in prefix:
            source = data.get('hist', [])
            domain = 'product'
            current_limit = 100 # From data/onerec_data/sft/product_rec.py
        elif "广告" in prefix:
            source = data.get('hist', [])
            domain = 'video'
            current_limit = 200 # From data/onerec_data/sft/ad_rec.py
        else:
            source = data.get('hist', [])
            domain = 'video' # Default
            current_limit = 512 # From data/onerec_data/sft/video_rec.py
            
        sids = converter.to_sid_str(source, domain, current_limit, deduplicate=True)
...
def main():
    ...
    try:
        ...
        # Limit training data for speed/memory in this test script
        print(f"Sampling training data to {INDEX_LIMIT} rows for quick index build...")
        df_train = df_train.head(INDEX_LIMIT)
    ...
    converter.save_mappings_json(OUTPUT_BASE_DIR)
    ...
    for task_name in ['video', 'ad', 'product']:
        ...
        if not os.path.exists(task_in):
            print(f"Skipping {task_name}, input dir not found.")
            continue
        ...
        for fname in os.listdir(task_in):
            ...
            # Limit benchmark rows for test
            print(f"Processing first {SAMPLE_LIMIT} rows of {fname}...")
            df_bench = df_bench.head(SAMPLE_LIMIT)
